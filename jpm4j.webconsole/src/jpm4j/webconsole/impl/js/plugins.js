...
(function() {
	...
	/**
	 * Local function wich is used as the main controller
	 * We retrieve 2 lists: a list of bundles and a list
	 * programs. The following functions manages the model
	 * of this.
	 */
	...
	function AqSearch($scope, $location, $http, $routeParams) {
		...
		function getBundle(bsn) {
			...
			if ( $scope.bundles) {
				for ( var i in $scope.bundles) {
					...
					if ( b.bsn === bsn )
						if ( !found || compareVersions(b.version, found.version) > 0)
							found = b;
				}
			}
			return found;
		...
		/*
		 * Setup the scope. Add a number of variables and functions 
		 * for the search.htm file
		 */
		$scope.query = $routeParams.query
		$scope.icon = function(program) {
			return (program.last.icon) || program.icon || "https://jpm4j.org/img/default-icon.png";
		}
	...
	angular.module( 'aq-console-plugins', []).config(function($routeProvider) {
		$routeProvider.
		when('/search/:query', { templateUrl: 'plugins/search.htm', controller: AqSearch }).
		when('/help', { templateUrl: 'plugins/help.htm', controller: AqHelp }).
